...
pub fn half_two() -> String {
    ...
    for line in string_input.split("\n") {
        if line.trim_start().starts_with('[') {
            ...
            let mut crates_to_move: LinkedList<char> = LinkedList::new();

            for _ in 0..moves.amt {
                crates_to_move.push_back(crates[moves.from - 1].pop_front().expect(""));
            }

            for element in crates_to_move.into_iter().rev() {
                crates[moves.to - 1].push_front(element)
            }
        }
    }
